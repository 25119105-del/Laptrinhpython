...
class MemoryGame:
    ...
    def draw(self):
        ...
        if self.scene == "MENU":
            ...
            # ===== TIÊU ĐỀ =====
            self.draw_text_title("FLIP GAME", (sw//2, 80))

            # ===== MÔ TẢ =====
            self.draw_text("Giáo viên hướng dẫn: ...", (sw//2, 160))
            self.draw_text("Thành viên: ...", (sw//2, 190))

            # ===== NÚT CHỦ ĐỀ (NẰM NGANG - 1 DÒNG RIÊNG) =====
            btn_w, btn_h = 160, 60
            gap = 20

            total_w = btn_w*3 + gap*2
            start_x = (sw - total_w) // 2
            y_btn = 260   # 👈 nằm dưới mô tả

            self.btn_amthuc = pygame.Rect(start_x, y_btn, btn_w, btn_h)
            self.btn_vanhoa = pygame.Rect(start_x + btn_w + gap, y_btn, btn_w, btn_h)
            self.btn_lichsu = pygame.Rect(start_x + 2*(btn_w + gap), y_btn, btn_w, btn_h)
            ...
            # ===== NÚT BẮT ĐẦU =====
            self.btn_start = pygame.Rect(sw//2 - 100, y_btn + 100, 200, 60)
