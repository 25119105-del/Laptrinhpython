...
THEME_BACKGROUNDS = {
    "Ẩm thực": "bg_am_thuc.jpg",
    "Văn hóa": "bg_van_hoa.jpg",
    "Lịch sử": "bg_lich_su.jpg"
}
class MemoryGame:
    def __init__(self):
        pygame.init()
        ...
        try:
            ...
            self.font_title =  pygame.font.SysFont("Arial", self.size_normal)
        ...
        try:
            ...
            self.bg_full = pygame.Surface((WIDTH, HEIGHT))
    ...
    def setup_level(self, theme):
        ...
        # Load background theo chủ đề
        bg_path = THEME_BACKGROUNDS.get(theme)

        if bg_path and os.path.exists(bg_path):
          self.bg_full = pygame.image.load(bg_path).convert()
        else:
          print(f"Không tìm thấy ảnh nền cho {theme}")
        # Lấy danh sách tên ảnh từ Database của theme đó
        # Scale background theo kích thước màn hình
        self.scale_bg(self.screen.get_size())
      ...
      # 4. TẢI ẢNH VÀO BỘ NHỚ
        ...
        for item_name in set(self.cards): 
            ...
            if image_path:
                ...
                print(f"LỖI: Chưa có ảnh cho '{item_name}' trong thư mục '{folder_name}'")
    ...
    def handle_click(self, pos):
        if self.scene == "MENU":
            ...
            elif hasattr(self, "btn_start") and self.btn_start.collidepoint(pos):
                if self.current_theme:
                    self.start_intro(self.current_theme)
    ...
    def draw(self):
        ...
        if self.scene == "MENU":
            ...
            # ===== THIẾT LẬP VÙNG BÊN PHẢI =====
            right_margin = sw * 0.1          # cách mép phải
            content_width = sw * 0.5         # vùng UI bên phải

            start_x = sw - content_width - right_margin

            # ===== TIÊU ĐỀ =====
            self.draw_text_title("FLIP GAME", (start_x + content_width//3, sh * 0.15))

            self.draw_text("Giáo viên hướng dẫn", (start_x + content_width//5, sh * 0.3))
            self.draw_text("Thành viên", (start_x + content_width//8, sh * 0.35))

            # ===== NÚT CHỦ ĐỀ (nằm ngang bên phải) =====
            btn_w = content_width * 0.3
            btn_h = sh * 0.15
            spacing = content_width * 0.05

            total_width = 3 * btn_w + 2 * spacing
            btn_start_x = start_x + (content_width - total_width) // 2
            y = sh * 0.55

            self.btn_amthuc = pygame.Rect(btn_start_x, y, btn_w, btn_h)
            self.btn_vanhoa = pygame.Rect(btn_start_x + btn_w + spacing, y, btn_w, btn_h)
            self.btn_lichsu = pygame.Rect(btn_start_x + 2*(btn_w + spacing), y, btn_w, btn_h)

            self.draw_button(self.btn_amthuc, (200,100,50), (255,150,80), "Ẩm thực")
            ...
            # ===== NÚT BẮT ĐẦU (dài bằng 3 nút) =====
            start_btn_width = total_width
            start_btn_height = btn_h
            ...
            self.draw_button(self.btn_start, (180,50,50), (255,100,100), "Bắt đầu")
